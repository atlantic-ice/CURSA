...
const DropZone = ({ children, sx: sxOverride, multiple }) => {
  ...
  const handleUpload = async (fileToUpload) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Произошла ошибка при загрузке файла. Повторите попытку.",
      );
    } finally {
      setLoading(false);
    }
            ...
            >
              ...
              {typeof children === "function"
                ? children({
                    ...
                    error,
                    openFile: () => fileInputRef.current && fileInputRef.current.click(),
                  })
                : children}
            </Box>
          </Paper>
  ...
  return (
    <Box>
      ...
      >
        <Paper
          elevation={0}
          sx={{ borderRadius: 60, overflow: "hidden", mb: 3, backgroundColor: "transparent" }}
        >
          <Box
            {...getRootProps()}
            sx={{
