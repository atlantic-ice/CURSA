...
def check_redis_health() -> Dict[str, Any]:
    """Проверяет доступность Redis, если он настроен."""
    if redis is None:
...
def health_detailed():
    ...
    status = (
        "healthy"
        if unhealthy_count == 0
        else ("degraded" if unhealthy_count < len(components) else "unhealthy")
    )
...
def readiness_probe():
    ...
    profiles_ok = (
        ...
        and len([f for f in os.listdir(profiles_dir) if f.endswith(".json")]) > 0
    )
