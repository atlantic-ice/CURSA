...
def _load_default_profile_data():
    ...
    try:
        ...
        current_app.logger.warning(f"Не удалось загрузить профиль ГОСТ: {exc}")
...
def _run_multipass_correction(file_path, original_filename, max_passes=3, verbose=False):
    ...
    if not os.path.exists(corrected_file_path):
        raise FileNotFoundError('Файл не был создан при исправлении')

    report_summary = report.get_summary()
    return {
        'success': True,
        'corrected_file_path': permanent_filename,
        'full_corrected_path': corrected_file_path,
        ...
        'report': {
            ...
            'actions_by_type': report_summary['actions_by_type'],
            'verification_results': report.verification_results,
        },
    }


def _preserve_original_for_preview(file_path, original_filename):
    source_name = original_filename or os.path.basename(file_path)
    base_name, _ = os.path.splitext(source_name)
    safe_base_name = secure_filename(base_name) or 'document'
    preview_timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    preview_filename = f"{safe_base_name}_original_{preview_timestamp}.docx"
    preview_path = os.path.join(CORRECTIONS_DIR, preview_filename)
...
def _build_improvement_summary(before_check_results, after_check_results):
    before_total = before_check_results.get('total_issues_count', 0)
    after_total = after_check_results.get('total_issues_count', 0)
    before_font = _count_font_issues(before_check_results)
    after_font = _count_font_issues(after_check_results)

    return {
        'before_total_issues': before_total,
        'after_total_issues': after_total,
        'resolved_total_issues': max(0, before_total - after_total),
        'before_font_issues': before_font,
        'after_font_issues': after_font,
        'resolved_font_issues': max(0, before_font - after_font),
    }
...
def _build_improvement_from_report(before_check_results, correction_report):
    before_total = before_check_results.get('total_issues_count', 0)
    before_font = _count_font_issues(before_check_results)
    remaining_issues = correction_report.get('remaining_issues', before_total)
    fixed_issues = correction_report.get('total_issues_fixed', 0)

    estimated_after_total = max(0, min(before_total, remaining_issues))
    estimated_resolved_total = max(0, before_total - estimated_after_total)
    estimated_resolved_font = min(before_font, fixed_issues)

    return {
        'before_total_issues': before_total,
        'after_total_issues': estimated_after_total,
        'resolved_total_issues': estimated_resolved_total,
        'before_font_issues': before_font,
        'after_font_issues': max(0, before_font - estimated_resolved_font),
        'resolved_font_issues': estimated_resolved_font,
    }


def _autocorrect_from_session(document_token, data):
    session = workflow_service.get_document_session(document_token)
    if not session:
        return jsonify({'error': 'Сессия документа не найдена или истекла'}), 404

    file_path = session.get('file_path')
    profile_id = session.get('profile_id')
    before_check_results = session.get('check_results') or {}
    original_filename = data.get('original_filename') or session.get(
        'original_filename', ''
    )
    ...
    current_app.logger.info(
        "Запуск автокоррекции по токену %s для файла %s",
        document_token,
        original_filename,
    )
    ...
    try:
        ...
        try:
            original_preview_path = _preserve_original_for_preview(
                file_path,
                original_filename,
            )
        except Exception as preview_exc:
            current_app.logger.warning(
                "Не удалось сохранить оригинал для preview: %s",
                preview_exc,
            )

        result = _run_multipass_correction(
            file_path,
            original_filename,
            max_passes=max_passes,
            verbose=verbose,
        ...
        full_corrected_path = result.get('full_corrected_path')
        corrected_filename = result.get('filename') or original_filename
        corrected_analysis = None

        if full_corrected_path:
            corrected_analysis = workflow_service.analyze_document(
                full_corrected_path,
                corrected_filename,
                profile_id,
            )

        if corrected_analysis and corrected_analysis.get('success'):
            corrected_check_results = corrected_analysis.get('check_results') or {}
            result['check_results'] = corrected_check_results
        ...
        result.pop('full_corrected_path', None)

        workflow_service.complete_document_session(document_token)
        ...
        current_app.logger.error(
            "Ошибка при автокоррекции документа по токену: %s: %s",
            type(exc).__name__,
            exc,
        )
        traceback.print_exc(file=sys.stdout)
        return jsonify({'error': f'Ошибка при исправлении документа: {str(exc)}'}), 500
...
def analyze_document():
    ...
    try:
        ...
        document_token = workflow_service.create_document_session(
            file_path,
            filename,
            profile_id,
            temp_dir,
...
def autocorrect_document():
    ...
    if not document_token:
        return jsonify({'error': 'Необходимо указать document_token'}), 400

    return _autocorrect_from_session(document_token, data)
